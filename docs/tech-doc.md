...
![Paper Bridge AWS 아키텍처](./diagrams/paper-bridge-architecture.drawio.png)

**워크플로**

![Paper Bridge 워크플로](./diagrams/paper-bridge-workflow.png)

**파이프라인**

![Paper Bridge 파이프라인](./diagrams/paper-bridge-pipeline.png)
...
![Paper Bridge 데이터 흐름](./diagrams/paper-bridge-dataflow.drawio.png)
...
├── assets/                        # 런타임 자산 + render-excalidraw.mjs
├── docs/
│   ├── tech-doc.md                # ← 이 파일
│   └── diagrams/                  # 아키텍처/워크플로/파이프라인/데이터흐름 (draw.io + Excalidraw)
├── eval/
...
제공합니다. 26개 테스트 모듈이 config, scorer, pipeline, Slack 블록/파일 업로드, text utils, 로거,
...
커버합니다. 현재 스위트는 **스킵 1건으로 ~377건이 통과**합니다. 공유 Neptune/OpenSearch 클라이언트는
2단계 삭제·공유 노드 보존(SUBJECT/OBJECT 엣지 방향 포함)·MemoryLimit 재시도·`delete_by_query` 등을 단위 테스트로 검증하며,
