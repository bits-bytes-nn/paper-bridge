...
YAML 설정에서 지정된 **활성** 모델은 응답/요약에 **Sonnet 4.6**, 추출과 그림 캡션에 **Haiku
4.5**입니다. `CLAUDE_V4_6_SONNET`은 `:N` 접미사가 없는 순수(bare) ID라는 점에 유의하세요.
...
**`Retriever`**는 동일한 Neptune + OpenSearch 저장소 위에 `LexicalGraphQueryEngine`을 래핑합니다.
그 전략은 전적으로 `config.retrieval`로 구동됩니다.

- **Traversal-based vs semantic-guided** (`traversal_based_or_semantic_guided`, 기본
  `traversal_based`):
  - `traversal_based` → `for_traversal_based_search(...)`; `set_subretriever`이면
    `[ChunkBasedSearch]`, 그 외에는 toolkit 기본값.
  - `semantic_guided` → `for_semantic_guided_search(...)`; `set_subretriever`이면
    `[StatementCosineSimilaritySearch, KeywordRankingSearch, SemanticBeamGraphSearch]`.
- **Reranking / post-processors** (`use_reranking_beam_search` / `use_post_processors`):
  reranker(`use_gpu_reranker`이면 `BGEReranker(gpu_id)`, 그 외에는 CPU `SentenceReranker`)에 더해
  선택적으로 `StatementDiversityPostProcessor`(`use_diversity`)와
  `StatementEnhancementPostProcessor`(`use_enhancement`)를 `_build_post_processors`가 조립합니다.
- **정확성 수정 — `_build_query_engine`:** 쿼리 엔진을 **단 한 번** 빌드하고, 선택된 베이스 모드
  (traversal 또는 semantic)의 팩토리에 `post_processors`를 전달합니다. 이전에는 각 `use_*` 메서드가
  `self.query_engine`을 재할당하여, reranking/post-processor를 켜면 traversal-based 엔진이 (deprecated된)
  semantic-guided 엔진으로 운영자 모르게 조용히 덮어써지는 문제가 있었습니다. 이제는 단일 엔진을
  빌드합니다.
- `query(query_text)`는 `max_chars=200000`으로 절단한 뒤 `query_engine.query(...)`를 실행하고
  `{response, source_nodes:[{text, metadata}]}`를 반환합니다.

**`PaperRetriever`**는 배치에 대한 검색을 오케스트레이션합니다.

- `DEFAULT_QUERIES` — 해당 분야의 최근 발전, 관련 연구 대비 핵심 차이, 방법론적 차이, 트렌드 인사이트를
  묻는 하나의 긴 쿼리(쿼리 시점에 논문 표현이 뒤에 덧붙음).
- 모델: `retrieval_summarization_model_id`(**Sonnet 4.6**), `BedrockConverse` 경유,
...
- 프롬프트: `RetrievalSummarizationPrompt.for_language_and_format(language, format)`. **출력 포맷이
  `SLACK`이면 언어가 `KO`로 강제**되고 Slack-markdown 변형(파이프 테이블 없음, `•` 불릿, `*bold*`,
  `[text](url)` 링크)이 사용됩니다. HTML 변형은 `<summary>` + `<urls>`를 생성합니다.
- **정확성 수정 — `retrieve_batch`:** `asyncio.gather(..., return_exceptions=True)`를 사용하여 한
  논문의 실패가 배치 전체의 검색 결과를 날리지 않도록 합니다(`summarize_batch`의 항목별 격리와 동일한
  방식). 논문×쿼리마다 하나의 `process_query`를 fan-out하고(동기 `Retriever.query`는 이벤트 루프를
  막지 않도록 `ThreadPoolExecutor`에서 실행), `arxiv_id`로 그룹화한 뒤 `process_response`가 검색 LLM을
  호출하고 `("summary","urls")`를 파싱합니다.
- **정확성 수정 — 쿼리 표현 축약:** `process_query` + `_build_query_representation`은 전체(~200k 문자)
  논문 대신 간결한 제목 + 초록(상한 `MAX_QUERY_PAPER_CHARS=2000`)을 검색 쿼리로 보냅니다. 전체 논문을
  보내면 Cohere 임베딩의 512-token 윈도를 약 100배 넘기고, 검색이 *관련* 연구가 아니라 논문 자신의
  진술 쪽으로 편향되기 때문입니다. retrieval 컨텍스트 prefix에는 `apply_cache_point`로 prompt caching이
  적용됩니다(§9.4).
